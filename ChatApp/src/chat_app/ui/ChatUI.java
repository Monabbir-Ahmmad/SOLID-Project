...
public class ChatUI extends JFrame {
    ...
    public ChatUI(String title) {
        setTitle(title);
        clientConnectionController = new ClientConnectionController(Constants.SERVER_IP, Constants.SERVER_PORT);
    ...
    private void initListeners() {
        ...
        clientController.setMessageReceiveListener((senderName, message, messageType, receivedFile) -> {
            if (messageType.equals(Constants.MESSAGE_TYPE_FILE) && receivedFile != null)
                receivedFileManager.addFileToList(receivedFile);

            createMessageLabel(true, senderName, message, messageType.equals(Constants.MESSAGE_TYPE_FILE));
        });
    ...
    private void sendTextMessage() {
        ...
        if (!text.isBlank()) {
            if (clientController.sendMsg(new TextMessageSender(text))) {
        ...
        if (fileChooser.showOpenDialog(null) == JFileChooser.APPROVE_OPTION) {
            ...
            if (dialogResult == JOptionPane.YES_OPTION) {
                if (fileChooser.getSelectedFile().length() > 0) {
                    if (clientController.sendMsg(new FileMessageSender(fileChooser.getSelectedFile())))
